'''
train.py
Updated: 9/25/17

README:

This script is used to train neural network models on the 2D encoded ModelNet10
dataset.

'''
import numpy as np
import glob
from models import *
from keras_extra import make_parallel_gpu
import os, time, sys; sys.path.insert(0, '../')
from keras.callbacks import CSVLogger, ModelCheckpoint
from keras.preprocessing.image import ImageDataGenerator

epochs = 100
batch_size = 10
model_def = CIFAR_NET
seed = 125
gpus = 1


################################################################################

if __name__ == '__main__':

    # File Paths
    os.chdir(os.path.dirname(os.path.realpath(__file__)))
    data_folder = "../../../data/split/AUG_ModelNet10_E2D"
    train_count = len(glob.glob1(data_folder+'/train', '*.png'))
    val_count = len(glob.glob1(data_folder+'/test', '*.png'))
    if not os.path.exists('logs'): os.makedirs('logs')
    if not os.path.exists('weights'): os.makedirs('weights')

    datagen = ImageDataGenerator()
    train_flow = datagen.flow_from_directory(data_folder +'/train', color_mode="grayscale",
                target_size=(512, 512), batch_size=batch_size, class_mode='categorical',
                seed=seed)
    validation_flow = datagen.flow_from_directory(data_folder +'/test', color_mode="grayscale",
                target_size=(512, 512), batch_size=batch_size, class_mode='categorical',
                seed=seed)

    # Load Model
    model, loss, optimizer, metrics = model_def(1, 10)
    if gpus > 1 : model = make_parallel_gpu(model, gpu)
    model.compile(loss=loss, optimizer=optimizer, metrics=metrics)

    # Callbacks
    date = time.strftime("%d%m%Y")
    csv_logger = CSVLogger('logs/'+model_def.__name__+date+".csv", separator=',')
    checkpointer = ModelCheckpoint(filepath='weights/'+model_def.__name__+date+'.hdf5',
                                   verbose=0, save_best_only=True)

    # Train Model
    model.fit_generator(train_flow, epochs=epochs, steps_per_epoch=train_count//batch_size,
                        validation_data=validation_flow, callbacks=[csv_logger,checkpointer],
                        validation_steps=val_count//batch_size)
