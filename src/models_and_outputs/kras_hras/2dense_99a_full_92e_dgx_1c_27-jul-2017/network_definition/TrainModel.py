...
data_folder_name = 'kras_hras/HH-512-MS-FULL'
image_size = (512, 512)
seed = 125
...
if __name__ == '__main__':
    ...
    train_flow = datagen.flow_from_directory(data_folder +'/train',
                target_size=image_size, batch_size=10, class_mode='categorical',
                seed=seed)
    test_flow = datagen.flow_from_directory(data_folder +'/test',
                target_size=image_size, batch_size=10, class_mode='categorical',
                seed=seed)
    save = ModelCheckpoint(output_folder + 'weights.hdf5', verbose=1, save_weights_only=True, period=5)

    # Fit Training Data
    if debug: print "Training Network..."
    history = network.model.fit_generator(train_flow, epochs=100,
            steps_per_epoch=9200,
                validation_data=test_flow, callbacks=[save,],
                validation_steps=2290)
