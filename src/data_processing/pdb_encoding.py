...
# Data folder path and hyper parameters
...
if __name__ == '__main__':
    ...
    # Parallelize tasks over cores
    if rank == 0:

        # Search for PDB class folders withing dataset folder
        tasks = []
        for class_fn in sorted(os.listdir(data_folder)):
            if os.path.isdir(data_folder+class_fn) and class_fn.lower().endswith('pdbs'):
                ...
                # Search for PDB files within class folders and add to list of tasks
                for pdb_fn in sorted(os.listdir(data_folder+class_fn)):

                    # Add augmentations to list
                    for i in range(nb_rot):
                        if chain is None:
                            tasks.append([data_folder+class_fn+'/'+pdb_fn, i, data_folder+save_fn+'/'+pdb_fn.split('.')[0]+'-r'+str(i)+'.png'])
                        else:
                            tasks.append([data_folder+class_fn+'/'+pdb_fn, i, data_folder+save_fn+'/'+pdb_fn.split('.')[0]+'_'+chain+'-r'+str(i)+'.png'])
    ...
    # Generate data array to file as defined by task
