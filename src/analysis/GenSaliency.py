...
encoded_folder = "KRASBOUNDED0%64-T45-MS-HH512"
if __name__ == '__main__':

    # Load Model
    net = CIFAR_512(nb_channels=3, nb_class=2, nb_gpu=1)
    net.model.load_weights("kras_hras_full-data_40-epochs_95A_weights.hdf5")

    # Load Data
    x = []
    pdb_imgs = []
    for line in sorted(os.listdir('../../data/processed/tars/'+encoded_folder + '/')):
        if line.endswith("r0.png"): pdb_imgs.append(line)
    ...
    for i in tqdm(range(len(pdb_imgs))):
        ...
        p = net.model.predict(img, batch_size=1, verbose=1)
        if np.argmax(p[0]) == 1:
            good += 1
            print p
            ...
            misc.imsave('KRAS_SALIENCY/'+pdb_imgs[i].split('.')[0]+'.png', atten_map)
    print good / float(len(pdb_imgs))
