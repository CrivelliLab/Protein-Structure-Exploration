...
from keras.layers import Conv2D, MaxPooling2D, Dropout, Input, Flatten, Dense, AveragePooling2D
...
def CIFAR_NET():
    ...
    optimizer = optimizer = Adadelta(decay=0.000001)
    metrics = [categorical_accuracy,]

    return model, loss, optimizer, metrics

def SIMPLENET_MODIFIED():
    '''
...
def VGG_16_REFERENCE_3KERN():
    ...
    model = Model(inputs=x, outputs=y)
    loss = categorical_crossentropy
    optimizer = optimizer = SGD(lr=0.00001, momentum=0.9, decay=0.0000001, nesterov=True)
    metrics = [categorical_accuracy,]

    return model, loss, optimizer, metrics

def VGG_16_REFERENCE_5KERN():
    '''
    Trainable Parameters: 287,352,642 (as reported by Keras on DGX-1).
                          138 Million (as reported by literature).
...
def SIMPLENET_DROPOUT():
    '''
    Trainable Parameters: 5.4 million (as reported by the cited paper).

    This network is based upon the SimpleNet architecture, which is a 13-layer
    design intended to attain state-of-the-art performance on the CIFAR10
    dataset while holding 2 to 25 times fewer parameters compared to previous
    deep architectures.
