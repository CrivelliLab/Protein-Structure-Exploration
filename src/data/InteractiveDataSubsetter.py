...
# TODO: tqdm display.
def split_classes(splits_dict, w_dir):
    '''
    DESCRIPTION: 
    ...
    INPUTS:
        "splits_dict", a dictionary containing 2 or three key/value pairs
            ...
            segmentation occurs. 
        "w_dir", a string containing the target or 'working' directory for the 
    ...
    create_directories(w_dir, split_dirs)
    print('Done')
...
def create_hdf5(splits_dict, images_shape_dict, sep_hdf5s_flag, w_dir):
    '''
    DESCRIPTION:
    ...
    if not sep_hdf5s_flag:
        single_file_name = (os.path.basename(w_dir) + '.hdf5') 
    ...
    if sep_hdf5s_flag:
        ...
        for slice_dict in slice_dicts: # E.g., 'train', 'test', 'validation'.
            ...
            hdf5_path = os.path.join(w_dir, dataset_name)
...
def get_subdirectories(target_dir):
    '''
    DESCRIPTION: 
        Gets the absolte paths of all subdirectories contained within
        this program's target directory. 
    INPUTS: 
        "target_dir" is a string containing the absolute path to the target
        directory. 
    RETURNS: 
        A list of absolute paths to all subdirectories in the CWD.
    '''
...
def untar_directories(w_dir):
    # TODO: fix tqdm display.
    '''
    DESCRIPTION: 
        Untars all ".tar.gz" files found in the current directory
            before segmentation or serialization processes begin. 
    INPUTS: 
        "w_dir", a string containing the target or 'working' directory for the 
            operations to be performed in. 
    RETURNS: 
        None. Decompresses all .tar.gzs in CWD. 
    '''
    print('Untaring all archives in directory. This may take some time...')
    try:
        i = 0
        num_tars = len([x for x in os.listdir(w_dir) if x.endswith('.tar.gz')])
        for file in tqdm(os.listdir(w_dir), total = num_tars):
...
def get_inputs():
    ...
    inputs_dict = {}
    inputs_dict['mode'] = None
    inputs_dict['sep_hdf5s_flag'] = False
    inputs_dict['tar_flag'] = False
