...
def display_3d_array(array_3d):
    ...
    for j in range(len(array_3d)):
        ...
        xx, yy, zz = np.where(array_3d[j] >= 0.75)
        ...
        for i in range(len(xx)):
            ...
            voxel_source.SetCenter(zz[i],yy[i],xx[i])
...
def display_3d_mesh(pdb_data, atten_array=None, dia=None):
    '''
    '''
    # Dislay 3D Mesh Rendering
    v = mlab.figure()
    ...
    if atten_array is not None:
        c = (1,1,1)

        # Coordinate Information
        xx, yy, zz = np.where(atten_array >= 0.8)
        res = (dia*2) / atten_array.shape[1]
        xx = (xx*res) - dia
        yy = (yy*res) - dia
        zz = (zz*res) - dia
        ...
        append_filter = vtk.vtkAppendPolyData()
        for i in range(len(xx)):
            #print xx[i], zz[i], yy[i]
            input1 = vtk.vtkPolyData()
            voxel_source = vtk.vtkCubeSource()
            voxel_source.SetCenter(zz[i], yy[i], xx[i])
            voxel_source.SetXLength(res)
            voxel_source.SetYLength(res)
            voxel_source.SetZLength(res)
            voxel_source.Update()
            input1.ShallowCopy(voxel_source.GetOutput())
            append_filter.AddInputData(input1)
        append_filter.Update()
